...
function getAllStructureCoordsFromLayout(layout, rcl) {
	...
	let positionsByType = layout[rcl] && layout[rcl].buildings;
	let coords = [];
...
export const bunkerCoordLookup = _.mapValues(
  _allBunkerCoords,
 (coordArr) =>
	 _.zipObject(_.map(coordArr, c => [coordName(c), true])),
);

// Fast function for checking if a position is inside the bunker
export function insideBunkerBounds(pos, room) {
	if (room.memory.bunkerData && room.memory.bunkerData.anchor) {
		let dx = bunkerLayout.data.anchor.x - room.memory.bunkerData.anchor.x;
		let dy = bunkerLayout.data.anchor.y - room.memory.bunkerData.anchor.y;
...
export function getPosFromBunkerCoord(coord, room) {
	if (room.memory.bunkerData && room.memory.bunkerData.anchor) {
		let dx = room.memory.bunkerData.anchor.x - bunkerLayout.data.anchor.x;
		let dy = room.memory.bunkerData.anchor.y - bunkerLayout.data.anchor.y;
...
export const bunkerChargingSpots = [{'x': 29, 'y': 24}, {'x': 24, 'y': 21}];
